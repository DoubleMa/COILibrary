...
namespace COILib.UI {
    ...
    public class Utils {
        ...
        public static Img GetImage(string pathOrUrl, string defaultAssets = Assets.Unity.UserInterface.General.Empty128_png) {
            Img img = new Img(defaultAssets);
            if (!string.IsNullOrEmpty(pathOrUrl)) UpdateImageOnMainThread(img, pathOrUrl);
            return img;
        }
        ...
        private static async void UpdateImageOnMainThread(Img img, string pathOrUrl) {
            try {
                ...
                if (imgData != null) RunOnMainThread(() => UpdateImage(img, imgData));
            }
            catch (Exception ex) {
        ...
        private static void UpdateImage(Img img, byte[] imgData) {
            Static.TryRun(() => {
                if (img == null || imgData == null) return;
                Texture2D texture = new Texture2D(2, 2);
                if (texture.LoadImage(imgData)) {
                    img.Image(texture);
                    img.RootElement.MarkDirtyRepaint();
                }
                else ExtLog.Error("Failed to load texture from image data");
            });
        }
    }
}