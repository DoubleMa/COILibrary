...
public abstract class AConfigManager<TL, T>(IniLoader loader) : InstanceObject<TL> where T : Enum where TL : AConfigManager<TL, T>, new() {
    private Dict<T, AIniKeyData> m_keyValues;
    protected readonly IniLoader m_loader = loader;

    protected readonly KeyCode[][] m_acceptedKeyCodes = [
